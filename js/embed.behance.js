	...
	function designTemplate(token, value) {
		...
		switch(token) {
			...
			if(settings.owners == true) {
				...
				dataWrapper += '<div class="wrap-label">Por: </div>';
	...
	function pagination(urlListNext) {
		...
		function paginationButton(action) {
			...
			if(action == 'show' && settings.infiniteScrolling == false) {
				...
				$(behanceContainer).append('<div class="eb-pagination-button" style="background-color: ' + settings.themeColor + '">Cargar Más... <span class="icon-loading"></span> </div>');
